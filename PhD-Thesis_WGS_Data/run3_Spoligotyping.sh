...
OUTDIR="/scratch/ma95362/musse_MGA/fastqs/MGA_paired_end_samples/results"
...
for file in "$OUTDIR"/*.spoligotype.json; do
    SAMPLE_NAME=$(basename "$file" .spoligotype.json)
    jq -r '. | to_entries | map([.key, .value] | join(",")) | join("\n")' "$file" > "$OUTDIR/$SAMPLE_NAME.spoligotype.csv"
done

# Print a message indicating completion
echo "Spoligotyping analysis and JSON-to-CSV conversion complete. Results are in $OUTDIR"
