...
df = pd.read_csv("resistance_level_gwas.txt", sep="\t")
...
sig_hits.to_csv("resistance_level_gwas_significant_hits.csv", index=False)

print(f"Total significant hits: {len(sig_hits)}")
print(f"Bonferroni threshold: {sig_threshold}")
print("Significant hits saved to resistance_level_gwas_significant_hits.csv")
print(sig_hits)
import os