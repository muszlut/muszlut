...
# Define file paths
# ---------------------------
gwas_dir <- "/scratch/ma95362/eth_national_analysis/all_fastq_reads/ETH_paired_end_samples/bactopia-runs/pangenome_local_all/pyseer_output"
gwas_file <- file.path(gwas_dir, "STR_EPTB_vs_PTb_gwas.txt")
counts_file <- file.path(gwas_dir, "STR_patterns_EPTB_PTb_counts.txt")
...
# ---------------------------
write.csv(gwas_data,
          file = file.path(gwas_dir, "STR_EPTB_PT_all_results.csv"),
          quote = FALSE, row.names = FALSE)

write.csv(sig_hits,
          file = file.path(gwas_dir, "STR_EPTB_PT_significant_hits.csv"),
          quote = FALSE, row.names = FALSE)
